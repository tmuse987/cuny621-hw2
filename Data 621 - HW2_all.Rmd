...
computeROCAUC <- function(df, class, probability)
{
    ...
    g <- g <- ggplot(data = dfXY, aes(x, y)) + xlab("1-Sensitivities")+ ylab("Specificity") + geom_line()
    spline.d <- as.data.frame(spline(dfXY$x, dfXY$y))
    
    g_smooth <- ggplot(data = spline.d, aes(x, y)) + xlab("1-Sensitivities")+ ylab("Specificity")  + geom_line() 
...
g1 <- ggplot(data.frame(sensitivities = rocCurve$sensitivities, specificities = rocCurve$specificities), aes(1-sensitivities, specificities))  +
    geom_line() +
