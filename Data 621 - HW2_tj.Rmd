...
The confusuion matrix is as follows, were again columns are actual class, and rows are predicted class (scored.class):
...
For the max value we know that the numerator of $F_{1}Score$ has a maximum of 2, that is Pecision*Sensitivity=1 then multiplied by 2. The demoninator's maximum value is also 2 as max Precision + max Sensitivity is 1+1. The the maximum value of $F_{1}Score$ is 1

$$F_1 Score = \frac {2 * 1 * 1}{1 + 1} = \frac{2}{2} = 1$$

For me minimum we simply observe that a Precision of Sensivity of zero will result in a $F_{1}Score$ of zero.
...
df$scored.probability <- as.numeric(df$scored.probability)
cd_roc <- roc(df, class, scored.probability)
cd_roc
plot(cd_roc)
...
rocCurve <- roc(response = class_data$class, 
                ...
                levels = rev(levels(class_data$class)))
auc(rocCurve)
ci(rocCurve)
